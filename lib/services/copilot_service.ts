...
function formatScheme(s: Scheme, intent: string, lang = 'en'): string {
  ...
  if (intent === 'ELIGIBILITY') {
    ...
    if (e && Object.keys(e).length > 0) {
      const eligItems: string[] = [];
      if (Array.isArray(e.occupations) && e.occupations.length > 0) eligItems.push(`Target Occupations: ${e.occupations.join(', ')}`);
      if (e.gender) eligItems.push(`Gender: ${e.gender}`);
      if (e.minAge !== undefined || e.maxAge !== undefined) {
        eligItems.push(`Age: ${e.minAge ?? 0} to ${e.maxAge ?? 'No limit'} years`);
      }
      if (e.maxAnnualIncome) eligItems.push(`Maximum Family Income: ₹${e.maxAnnualIncome.toLocaleString('en-IN')}/year`);
      if (e.residentialStatus) eligItems.push(`Residential Status: ${e.residentialStatus}`);
      if (Array.isArray(e.customRules) && e.customRules.length > 0) {
        eligItems.push('Criteria:\n' + (e.customRules as string[]).map((r: string) => `• ${r}`).join('\n'));
      }
      if (e.note) eligItems.push(`Note: ${e.note}`);
      parts.push(`${lbl.elig}:\n` + eligItems.join('\n'));
    } else {
      parts.push(`${lbl.elig}: Open to all eligible Indian citizens meeting official criteria.`);
    }
  }

  if (intent === 'REQUIRED_DOCUMENTS') {
    if (s.documents && s.documents.length > 0) {
      parts.push(`${lbl.docs}:\n` + s.documents.map(d => `• ${d.name}${d.description ? ` - ${d.description}` : ''}`).join('\n'));
    } else {
      parts.push(`${lbl.docs}: Standard government identity verification (Aadhaar/e-KYC) required.`);
    }
  }

  if (intent === 'APPLICATION_PROCESS') {
    if (s.benefits && s.benefits.length > 0) {
      parts.push(`Application Steps:\n` + s.benefits.map((step: string, idx: number) => `${idx + 1}. ${step}`).join('\n'));
    }
    if (s.documents && s.documents.length > 0) {
      parts.push(`Key Documents Required: ` + s.documents.map(d => d.name).join(', '));
    }
    if (s.application_url || s.official_url) {
      parts.push(`${lbl.apply}: ` + (s.application_url || s.official_url));
    }
  } else if (intent === 'OFFICIAL_PORTAL' && (s.application_url || s.official_url)) {
    parts.push(`${lbl.apply}: ` + (s.application_url || s.official_url));
  }
